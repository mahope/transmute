...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **atten** færdige commits på tolv branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; hvis `DEPLOY-MISSING` er væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a`, T24 `ca259c1`, T25 `5fd4648`, T26 `b20b809`, T27 `0abe7a3`, T28 `763efbc`, T29 `e156dec` og T30) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, er den næste opgave en **ny måling**, ikke de gamle: filstier med mellemrum i `--out`, POSIX-locale (`LC_ALL=C` med ikke-ASCII), store filer (≥ 100.000 rækker — er der en sti, der bliver langsom eller rammer en grænse, som `Limits`-afsnittet lover, at der ikke er?) og YAML-mappingsdybde. Den skal måles på den rigtige binary og fundene skrives ned, også når de er *ikke* fejl — T30 skrev to målte ikke-rettelser ned (punkt 9 og 10 under `❓ Til Mads`), og det er dem, der gør at næste iteration ikke måler dem igen.
...
**Den elvte flade er ikke målt endnu.** Efter ti målte tavsheder er der ingen ny fejl i køen, kun det uudforskede: de stier, ingen af de ti opgaver rørte. T30 målte dem, og den efterlod præcis to ting, som er skrevet ned som målte og bevidst urørte (se `❓ Til Mads` punkt 9 og punkt 10) — så det, der ikke er målt, er nu **filstier med mellemrum i `--out`, POSIX-locale, store filer (≥ 100.000 rækker) og YAML-mappingsdybde**.
...
**Status:** FÆRDIG med commit `PLACEHOLDER` på `ceo/utf8-input` — **ligger på branch, ikke mergeret**, fordi `DEPLOY-MISSING` står. Se Deploy.
**Mislykkede forsøg:** 0/2

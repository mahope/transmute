...
**Deploy-status ⚠️ (genverificeret 2026-09-26 med `npm run check:deploy`, starten på T43-iterationen):** uændret for **ottende** gang i træk. Live er `3d90812` (2026-09-24 23:32:50 +0200), 5 site-commit i drift, 25 afvigende filer, `/support/index.html` utilgængelig. Se `❓ Til Mads` punkt 1.
...
**Næste iteration:** T44 er færdig (se punkt 44) på `ceo/xml-list-shape`, bygget oven på `ceo/csv-header-name`, med samme grund: rører `site/engine.js`. Den er den **første iteration med en hård fejl i stedet for en tavshed**: `json → xml` exit 0 og den følgende `xml → json` exit 3 på den fil, værktøjet selv lige havde skrevet. Deploy-stillen er uændret, så bunken er nu **enogtyveogtyve** færdige commits på toogtyve branch uden merge. Kør `npm run check:deploy` først; er `DEPLOY-MISSING` væk, merges hele bunken til `main` og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal næste opgave være **T45**, og den skal begynde med de **tre fund T44 målte og bevidst ikke rettede** (se punkt 44), fordi de er samlet i én beslutning om, hvad en liste må blive til i XML.

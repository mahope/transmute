...
**Deploy-status ⚠️ (genverificeret 2026-09-26 med `npm run check:deploy`, starten på T45-iterationen):** uændret for **tiende** gang i træk. Live er `3d90812` (2026-09-24 23:32:50 +0200), 5 site-commit i drift, 25 afvigende filer, `/support/index.html` utilgængelig. Se `❓ Til Mads` punkt 1.
...
**Næste iteration:** T45 er færdig (se punkt 45) på `ceo/xml-list-roundtrip`, bygget oven på `ceo/xml-list-shape`, med samme grund: rører `site/engine.js`. Den lukker **den ene beslutning T44 skrev ned og bevidst lod ligge** — hvad en liste må blive til i XML — og dens fund var **større end T44's egne tre**: en liste blev skrevet som N søskendeelementer der alle hed `<field>`, så enhver læser uden for dette værktøj beholdt **kun det sidste medlem**. Deploy-stillen er uændret, så bunken er nu **syvogtyve** færdige commits på **enogtyve** branch uden merge. Kør `npm run check:deploy` først; er `DEPLOY-MISSING` væk, merges hele bunken til `main` og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal næste opgave være **T46**, målt før den skrives, i den flade T45 skrev som sit mål: `parseElement` læser et gentaget barns nøgle til et array, så to ens nøgler i en håndskrevet XML-fil bliver en liste — den sidste sti hvor læseren vælger mellem to forpligtelser, aldrig målt med to dommere.

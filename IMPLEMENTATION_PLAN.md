...
**DEPLOY-MISSING: batchdeployeren har ikke kørt siden 2026-09-08 — genverificeret 2026-09-26 ca. 02:4x CEST.** `https://transmute.run/sitemap.xml` har stadig `lastmod 2026-09-08` på alle entries, `https://transmute.run/support/` svarer stadig 404, og forsiden har 0 forekomster af `href="/support/"`. Det er langt over to deploy-vinduer siden T7's første note.

**Konsekvens for merges, ændret i denne iteration.** Tidligere iterationer fortsatte med at merge, fordi T10 rørte ingen `site/`-fil og derfor hverken kunne rette eller forværre deployen. Det er ikke længere holdbart at gå ud fra: T13 er en reel **fejlrettelse i `site/engine.js`**, altså en fil der kun kan få glæde af at blive deployet. Reglen er derfor nu skrevet som den er formuleret i kontrakten — **loopet merger ikke til `main`, mens `DEPLOY-MISSING` står** — og T13 ligger derfor færdig, testet og grøn på `ceo/csv-real-world-fixes`. Det er en bevidst afvejning: en grøn, ubrugt rettelse er billigere end en ny deploy-note oven i en forvirring, og intet går tabt, fordi branchen er pushet.

**T12 (dansk support-side) er derfor også parkeret** — det er sitearbejde, og sitearbejde under en død batchdeployer er spildt. Den næste gang Mads svarer på punkt 1 og deployen er genstartet, kan T12 og T13's sitehalvdel merges samlet.
...
**Status:** TODO, men **venter på Mads** — se afsnittet Deploy
**Mislykkede forsøg:** 0/2
...
### 14. [ ] Håndter CSV-rækker med flere felter end overskriften

**Status:** TODO
**Begrundelse:** Fund under T13. `parseCSV` bygger rækker med `headers.forEach(...)`, så en CSV-række med **flere** felter end overskriften taber de ekstra værdier tavst, og en med færre fylder resten med `''`. T13 stoppede ved de tre fund, der krævede en ny læser; dette kræver en produktafgerelse, fordi "tab de ekstra" og "fejl på de ekstra" er to forskellige valg.
**Scope:**

- Vælg adfærd: ignorer, opkald feltet efter et dokumenteret mønster, eller exit 3 med tydelig besked. Skriv valget i `docs/cli.md` og dæk det med en test.
- Overvej det samme for YAML- og XML-input, der har samme record-form men andre parseveje.
...
1. **Batchdeployeren kører tilsyneladende ikke, og det har nu kostet en merge.** Live-sitet er fra 2026-09-08: `sitemap.xml` har `lastmod 2026-09-08`, `/support/` er 404, og forsiden mangler `/support`-linket, selv om T7 blev pushet 2026-09-25. Cloudflare serverer dynamisk (`max-age=0, must-revalidate`), så det er ikke en cache. Efter T5's fjernelse af `deploy-site.yml` er der ingen mekanisme tilbage i repoet, der deployer. **T13 er færdig, grøn og ligger på `ceo/csv-real-world-fixes` uden at være mergeret, fordi den rører `site/engine.js`** — se Deploy. Det er første gang denne blokering faktisk har stoppet en færdig rettelse, så beslutningen er ikke længere hypotetisk: genstartes batchdeployeren, eller skal sitet deployes en anden vej? Loopet deployer aldrig selv.

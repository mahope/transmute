...
**Deploy-status ⚠️ (genverificeret 2026-09-26 08:0x CEST med `npm run check:deploy`, starten på T31-iterationen):** uændret for **sjette** gang i træk. Live er `3d90812` (2026-09-24 23:32:50 +0200), 5 site-commit i drift, 25 afvigende filer, `/support/index.html` utilgængelig. Se `❓ Til Mads` punkt 1.
...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **nitten** færdige commits på tretten branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; er `DEPLOY-MISSING` væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a`, T24 `ca259c1`, T25 `5fd4648`, T26 `b20b809`, T27 `0abe7a3`, T28 `763efbc`, T29 `e156dec`, T30 `837eb4a` og T31 `c5e7d4b`) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal den næste opgave være en **ny måling** — de fire stier T31 målte er lukket, og de er skrevet ned som målte, også når de ikke var fejl, så ingen bør måle dem igen. Den naturlige næste flade er den samme slags, T31's fund peger på: **de skærmformater, der ikke er `table`** — `xml` på brede tegn (tegn der ikke er gyldige XML-navne, håndterer T21; men *værdier* er ikke prøvet af), `sql` på de samme tegn, og `csv` på en celle der er længere end nogen anden. Den skal måles på den rigtige binary og fundene skrives ned, også når de ikke er fejl.

...
**Næste iteration:** T12 — udgiv support-siden på dansk (`IMPLEMENTATION_PLAN.md`, opgave 12). Den lå og vente på Mads' prioritering mod T9 og T10, og begge er nu færdige, så T12 er den eneste åbne opgave, der ikke er blokeret af en beslutning. Først at gøre hver iteration: `npm run check:deploy` — hvis `DEPLOY-MISSING` er væk, merges de **seks** commits til `main` (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce` og T17) og `VERIFICÉR DEPLOY`-noterne lukkes. Uden svar på `❓ Til Mads` punkt 1 vokser bunken, så det er den note der frigør mest.

...
**Status:** I GANG — slice 1, 2, 3, 4 og 5 færdige (`14dce0b`, `599ca4f`, `a4114ca`, `8a161fb` og `bb19768`). Slice 6 (`actions/setup-node` v5 → v6) er færdig i dette commit. Slice 7+ er de resterende to action-majors, én pr. commit.
**Mislykkede forsøg:** 0/2
...
| 6 | `actions/setup-node` v5 → v6 | samme | **Fændig** (dette commit). Gulvet hævet til 6; `always-auth` fjernet og caching-triggeren bredere, begge inerte og dækket af en ny kontrol |
| 7 | `actions/checkout` v6 → v7 | samme | Næste iteration. Hæv `checkout`-gulvet til 7 i samme commit; slutter på samme pin som `site-gate.yml` bruger i dag |

...
**Deploy-status ⚠️ (genverificeret 2026-09-26 med `npm run check:deploy`, starten på T41-iterationen):** uændret for **sekstende** gang i træk. Live er `3d90812` (2026-09-24 23:32:50 +0200), 5 site-commit i drift, 25 afvigende filer, `/support/index.html` utilgængelig. Se `❓ Til Mads` punkt 1.
...
**Næste iteration:** T41 er færdig (se punkt 41) på `ceo/csv-scalar-types`, bygget oven på `ceo/yaml-string-scalars`, med samme grund: rører `site/engine.js`. Den **lukker den anden ende af skriverens skalare** — `csv`, `table` og `xml` på værdier der ligner tal og typer — og den gav **tre fund i én funktion**, hvoraf det første er den værste tavshed i hele rækken siden T30: en post med en tom værdi i en énkolonne-CSV blev skrevet som den tomme linje RFC 4180 tillader, læseren spiste den, og **tre records ind, to ud, exit 0, tom stderr**. Deploy-stillen er uændret, så bunken er nu **toogtyve** færdige commits på enogtyve branch uden merge. Kør `npm run check:deploy` først; er `DEPLOY-MISSING` væk, merges hele bunken til `main` og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal næste opgave være **T42**, målt før den skrives, og den skal **ikke** genåbne punkt 16 i `❓ Til Mads` (en beslutning, ikke en flade) eller punkt 15's tabsfri læser (en ændret værdi, ikke en målt tavshed). Den næste ubeskrevne flade er den **tredje** ende, som T41's måling bevidst lod ligge: **`table` og `sql` på værdier der ikke kan skrives i den celle de får** — en celle med et linjeskift, en `|` eller en `+---+` der ligner en tabelramme. T41 målte `table` på *type* og fandt nul, fordi en terminal ikke typetester; den sag er en anden, og den er ikke målt endnu.

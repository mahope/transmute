...
**Næste iteration:** T43 er færdig (se punkt 43) på `ceo/csv-header-name`, bygget oven på `ceo/table-cell-sql-ident`, med samme grund: rører `site/engine.js`. Den lukkede den **fjerde** ende af skriverens celle — den T42 skrev som den eneste udokumenterede — og målte **26 varianter for at finde tre fund i én funktion**. Deploy-stillen er uændret, så bunken er nu **toogtyve** færdige commits på enogtyve branch uden merge. Kør `npm run check:deploy` først; er `DEPLOY-MISSING` væk, merges hele bunken til `main` og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal næste opgave være **T44**, målt før den skrives, og den skal **ikke** genåbne punkt 16 i `❓ Til Mads` (en beslutning, ikke en flade) eller punkt 15's tabsfri læser (en ændret værdi, ikke en målt tavshed).

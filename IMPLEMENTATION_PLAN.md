...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger otte færdige commits på branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; hvis `DEPLOY-MISSING` er væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643` og T12) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, er det nye produktarbejde, der skal vælges: `❓ Til Mads` punkt 1 er den eneste beslutning, der frigør mest, og intet i køen kan løse den.
...
**Status:** FÆRDIG på `ceo/da-support` — **ligger på branch, ikke mergeret**, fordi diffen rører `site/` og `DEPLOY-MISSING` står. Se Deploy.
**Mislykkede forsøg:** 0/2

...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **tolv** færdige commits på seks branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; hvis `DEPLOY-MISSING` er væk, merger hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a` og T24 `ca259c1`) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, er det nye produktarbejde, der skal vælges, og **T20's metode er den der virkede**: kør rigtige filer gennem `src/cli.js` med hvert output-format, og se efter exit 0 med mistet data.
...
**Den næste flade er den, T23's research efterlod: `run`s egen fejlklassificering.** `run` returnerer i dag `{ error }` og skelner ikke på, om fejlen kom fra input, pipeline eller serialisering; kun `parsePipeline` i `cli.js` går forud og giver exit 2 for ukendte ops. Konkrete ting at undersøge, alle fundet ved at læse koden efter T23's rettelse: (a) `parsePipeline` kræver et `op`-nøgle, men **ikke** at `filter`/`map` har en `expr` — `{"op":"filter"}` er stadig identiteten og dermed stadig "alle rækker, exit 0", altså T23's fund med en anden vej ind; (b) `head`/`tail` med `n` som tekst (`{"op":"head","n":"5"}`) rammer `slice` med en streng, fordi intet validerer typen; (c) `sort` med `by` på et felt ingen rækker har, sorterer uden at sige det. Det er den samme familie som T13–T24: **exit 0 med et svar, brugeren ikke bad om.** Start med (a), fordi den er den direkte søster til T23's fix.

`npm run check:site` er grøn efter T23 (20 sider, 0 fund, 0 layout-afvigelser). `❓ Til Mads` punkt 1 er stadig den eneste beslutning, der frigør mest, og intet i køen kan løse den.

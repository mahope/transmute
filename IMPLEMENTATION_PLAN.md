...
**Deploy-status ⚠️ (genverificeret 2026-09-26 08:0x CEST med `npm run check:deploy`, starten på T32-iterationen):** uændret for **syvende** gang i træk. Live er `3d90812` (2026-09-24 23:32:50 +0200), 5 site-commit i drift, 25 afvigende filer, `/support/index.html` utilgængelig. Se `❓ Til Mads` punkt 1.
...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **tyve** færdige commits på femten branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; er `DEPLOY-MISSING` væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12 `6516fdb`, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a`, T24 `ca259c1`, T25 `5fd4648`, T26 `b20b809`, T27 `0abe7a3`, T28 `763efbc`, T29 `e156dec`, T30 `837eb4a`, T31 `c5e7d4b`, T32 `88ea6f6` og T33 `46b5567`) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal den næste opgave være **T34**, de to læserhuller T33 målte og bevidst lod ligge: en YAML-flowsamling som hele dokumentet læses som én streng, og to XML-dokumenter med samme rodnavn fanges kun af elementkontrollen, så rodreglens besked ikke kommer frem. Begge er skrevet ned med scope og acceptkriterier, og ingen af dem er en tavs datatab.
...
### 34. [ ] Gør de to målte huller i læserne tætte
...
1. **En flowsamling som hele dokumentet.** `{a: 1, a: 2}` på roden læses som den bogstavlige streng `"{a": "1, a: 2}"` — exit 0, filens indhold mangle til ét felt. `parseYAMLFlow` findes allerede og læser den samme tekst korrekt nede i filen, så rettelsen er at lade `parseYAML` prøve den på dokumentroden, når første linje starter med `{` eller `[`. Bevar: kun når flowet lukker på den linje, så en ubalanceret `{` ikke ændrer dagens adfærd.
2. **Rodens afsluttende tag skal findes dybde-aware.** `lastIndexOf('</rows>')` finder den *sidste* `</rows>`, så to dokumenter med samme rodnavn — den form et batchværktøj der appender samme skema faktisk skriver — ikke fanges af efter-rod-reglen. Tælle åbne og lukkede rodelementer i stedet for at tage den sidste. Forsigtighed: `<rootTag/>` er selv Lukket, og et rodnavn må ikke matche et længere navn (`<roots>` er ikke `<root>`).

**Acceptkriterier:**

- `transmute -f yaml` på `{a: 1, a: 2}` (og `[1, 2, 3]`) på roden læses som sit indhold, ikke som én streng; en test låser, at en ubalanceret `{` på roden stadig læses som den gør i dag.
- `<rows>a</rows><rows>b</rows>` og `<rows><rows/></rows>` er begge exit 3 med **rodreglens** besked, ikke elementkontrollens.
- `<root><root>x</root></root>` (samme navn indeni) læses stadig korrekt — det er den regression dybdetællingen kan lave.
- 0 af de eksisterende snapshots ændrer sig.
- `site/engine.js` regenereres, og `npm run check:site` er grøn.
...
13. **To læserhuller er målt og ikke rettet (fra T33).** (a) En YAML-flowsamling som *hele dokumentet* — `{a: 1, a: 2}` på roden — læses som den bogstavlige streng `"{a": "1, a: 2}"`, exit 0, altså filens indhold mangle til ét felt. Den er kun nåelig med eksplicit `-f yaml`, da `detectFormat` sender `{` til JSON-læseren, som med vilje afviser den. (b) To XML-dokumenter med **samme** rodnavn — den form et batchværktøj, der appender samme skema, faktisk skriver — fanges ikke af den nye rodregel, fordi `lastIndexOf` tager den *sidste* `</rows>`; de afvises dog stadig med exit 3 og tom stdout, blot med en besked der peger på det indre i stedet for på rodreglen. Begge ligger i T34 med scope og acceptkriterier, og ingen af dem er en tavs datatab. **Ingen beslutning nødvendig** — kun til gengældelse, hvis du vil have dybde-aware rodsøgning gjort først, fordi den er den eneste af de to, der kan træffe en eksisterende, virkende fil.

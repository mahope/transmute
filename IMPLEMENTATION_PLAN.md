...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **enogtyve** færdige commits på seksten branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; er `DEPLOY-MISSING` væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12 `6516fdb`, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a`, T24 `ca259c1`, T25 `5fd4648`, T26 `b20b809`, T27 `0abe7a3`, T28 `763efbc`, T29 `e156dec`, T30 `837eb4a`, T31 `c5e7d4b`, T32 `88ea6f6`, T33 `46b5567` og T34 på `ceo/reader-gaps`) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal den næste opgave være **T35**: **advarslerne fra de sidste tre iterationer når aldrig en browserbruger.** `site/try.html` videresender `r.error` men ikke `r.warnings`, så playgroundet på `/` og `/da/` — den mest brugte flade i produktet, og den man netop bruger til at prøve en pipeline — giver `{}` for en række efter en `pick` med en tastefejl, `{a: 1, a: 2}` som helt dokument, `unique --by` på et felt ingen rækker har og tre målinger til, uden at advarslen fra CLI'en når ham. T26 skrev det som `❓ Til Mads` punkt 8; efter T33 og T34 er det ikke længere en detalje, det er **fladen hvor advarslerne dør**, og den skal måles på den rigtige motor (`site/engine.js`) inden der røres.
...
**Status:** FÆRDIG med implementationscommit på `ceo/reader-gaps` (bygget oven på `ceo/reader-ambiguity`) — **ligger på branch, ikke mergeret**, fordi diffen rører `site/engine.js` og `DEPLOY-MISSING` står. Se Deploy.
**Mislykkede forsøg:** 0/2
...
14. **Browserplaygroundet får ingen advarsler — fra `❓` punkt 8, hævet af T34 (næste opgave, T35).** Det var en sideeffekt, da T26 skrev det ned; efter T33 og T34 er advarslerne en *dokumenteret* del af læserne, og de dør i den flade, flest brugere ser. Ikke en beslutning, men den bør stå øverst i køen, fordi den er den eneste, der både er lille og rammer den mest brugte side.

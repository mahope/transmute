...
**Næste iteration:** deploy-stillen er uændret, og derfor ligger **tyve** færdige commits på fjorten branch uden merge. Den næste iteration skal **først** køre `npm run check:deploy`; er `DEPLOY-MISSING` væk, merges hele bunken (T13 `4d7fc35`, T14 `5f64894`, T15 `2fb9631`, T16 `bf61cce`, T17 `8be3643`, T12, T18 `a31aba8`, T20 `dbeae3c`, T21 `f49458a`, T22 `0115204`, T23 `29f1e9a`, T24 `ca259c1`, T25 `5fd4648`, T26 `b20b809`, T27 `0abe7a3`, T28 `763efbc`, T29 `e156dec`, T30 `837eb4a`, T31 `c5e7d4b` og T32) til `main`, og `VERIFICÉR DEPLOY`-noterne lukkes. Står den stadig, skal den næste opgave være en **ny måling**. T32's målingstabel er skrevet ned, også for de fund der ikke var fejl, så de måles ikke igen: **de fire skærmformatters brede tegn og én lang celle er lukket** (rene), og **XML 1.0's `Char`-produktion er lukket for alle otte ulovlige værdityper, feltnavne og attributter** (én fejl, rettet). Den næste flade er den samme slags og ligger i de stier T32 ikke rørte: de **andre** læsere. T30 målte kun encodingsfejlen på *ind*, aldrig det en læser gør med en fil den ikke forstår — en CSV med en feltantal-kollision, en YAML-fil med en duplikeret nøgle, en XML-fil med to rødder — for de læsere har alle exit 0 som deres dårligste svar, og T14/T22 viste at de to CSV-svagheder, der blev fundet, lå i den slags.
